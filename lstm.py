...
class LSTM(nn.Module):
    def __init__(self, configs, device, loss_function, scaler, save_path):
    ...
    def train_online_sgd(self, x, y, optimizer):
        self.train()
        losses = []
        times = []
        ...
        for batch_idx in range(len_batch):
            ...
            if batch_idx % 20 == 0:
                ...
                print('[SGD online learning: {}/{} ({:.0f}%)]\tLoss: {:.6f},'
                      ' time spent:{:.6f} s'.format(batch_idx * self.batch_size, x.size(0),
                                                    100. * batch_idx / len_batch,
                                                    test_loss, duration))
        torch.save(self.state_dict(), self.path)
        return losses, times

    def train_online_ptssgd(self, x, y, optimizer):
        self.train()
        losses = []
        times = []
        len_batch = math.ceil(x.size(0) / self.batch_size)
        for batch_idx in range(len_batch):
            ...
            if batch_idx % 20 == 0:
                ...
                losses.append(test_loss.item())
                times.append(duration)
                print('[PTSSGD online learning: {}/{} ({:.0f}%)]\tLoss: {:.6f},'
                      ' time spent:{:.6f} s'.format(batch_idx * self.batch_size, x.size(0),
                                                    100. * batch_idx / len_batch,
                                                    test_loss, duration))
        torch.save(self.state_dict(), self.path)
    ...
    def train_online_htssgd(self, x, y, w, optimizer):
        self.train()
        losses = []
        times = []
        loss_list = []
        ...
        for batch_idx in range(len_batch):
            ...
            if batch_idx % 20 == 0:
                ...
                losses.append(test_loss.item())
                times.append(duration)
                print('[HTSSGD online learning: {}/{} ({:.0f}%)]\tLoss: {:.6f},'
                      ' time spent:{:.6f} s'.format(batch_idx * self.batch_size, x.size(0),
                                                    100. * batch_idx / len_batch,
                                                    test_loss, duration))
        torch.save(self.state_dict(), self.path)
    ...
    def train_offline_sgd(self, x, y, optimizer):
        self.train()
        losses = []
        times = []
        len_batch = math.ceil(x.size(0) / self.batch_size)
        for batch_idx in range(len_batch):
            ...
            if batch_idx % 20 == 0:
                ...
                losses.append(test_loss.item())
                times.append(duration)
                print('[SGD offline learning: {}/{} ({:.0f}%)]\tLoss: {:.6f},'
                      ' time spent:{:.6f} s'.format(batch_idx * self.batch_size, x.size(0),
                                                    100. * batch_idx / len_batch,
                                                    test_loss, duration))
        torch.save(self.state_dict(), self.path)
